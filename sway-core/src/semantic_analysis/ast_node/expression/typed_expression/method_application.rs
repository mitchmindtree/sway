    ...
    let method = match method_name {
        MethodName::FromType {
            ...
            let abs_path: Vec<Ident> = if call_path.is_absolute {
                ...
                scope.iter().chain(call_path.full_path()).cloned().collect()
            };
            check!(
        ...
        MethodName::FromModule { ref method_name } => {
            ...
            let abs_path: Vec<_> = scope.iter().chain(Some(method_name)).cloned().collect();
            check!(
                root.find_method_for_type(ty, &abs_path, self_type, &args_buf),
                return err(warnings, errors),
                warnings,
                errors
            )

...
impl TypedExpression {
    ...
    fn type_check_function_application(
        ...
        println!("Before `get_call_path`");
        let function_declaration = check!(
            root.get_call_path(scope, &name),
            return err(warnings, errors),
            warnings,
            errors
        );
        println!("After `get_call_path`");
    ...
    fn type_check_if_let_expression(
        ...
        let (enum_type, variant) = check!(
            check_scrutinee_type(&scrutinee, root, scope),
            return err(warnings, errors),
            warnings,
            errors
        );
    ...
    fn type_check_asm_expression(
        ...
        let return_type = check!(
            root[scope].resolve_type_with_self(asm.return_type.clone(), self_type, asm_span, false),
            insert_type(TypeInfo::ErrorRecovery),
            warnings,
            errors,
        );
    ...
    fn type_check_subfield_expression(
        ...
        let (fields, struct_name) = check!(
            root[scope].get_struct_type_fields(
                parent.return_type,
                parent.span.as_str(),
                &parent.span
            ),
    ...
    fn type_check_tuple_index(
        ...
        let tuple_elems = check!(
            root[scope].get_tuple_elems(parent.return_type, parent.span.as_str(), &parent.span),
            return err(warnings, errors),
            warnings,
            errors
        );
    ...
    fn type_check_delineated_path(
        ...
        let exp = if let Some(enum_decl) = root[scope]
        ...
        } else if let Some(module) = root[scope]
    ...
    fn type_check_abi_cast(
        ...
        let abi = check!(
            root.get_call_path(scope, &abi_name),
            return err(warnings, errors),
            warnings,
            errors
        );
    ...
    fn type_check_get_type_property(
        ...
        let type_id = check!(
            root[scope].resolve_type_with_self(type_name, self_type, type_span, true),
            insert_type(TypeInfo::ErrorRecovery),
            warnings,
            errors,
        );
...
fn check_scrutinee_type(
    ...
    let (ty, enum_variant) = match scrutinee {
        Scrutinee::EnumScrutinee { ref call_path, .. } => check!(
            check_enum_scrutinee_type(call_path, root, scope),
            return err(warnings, errors),
            warnings,
            errors
        ),
...
fn check_enum_scrutinee_type(
    ...
    let decl: TypedDeclaration = check!(
        root.get_call_path(scope, &call_path),
        return err(warnings, errors),
        warnings,
        errors
    );
    ...
    let enum_decl = if !enum_decl.type_parameters.is_empty() {
        enum_decl.monomorphize(&mut root[scope])
    } else {
        enum_decl
    };
